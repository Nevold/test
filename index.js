...
color.addEventListener('change', async (event) => {
  ...
  let { item, count } = await getCars(1);
  carName.innerText = item[0].name;
  item.map((car) => (carName.innerText = car.name));
});
...
(async () => {
  ...
  <div class="control-button">
              <button class="button" id="select-car-${car.id}">Select</button>
              <button class="button" id="remove-car-${car.id}">Remove</button>
              <span class="car-name">${car.name}</span>
            </div>
            <div class="road">
              <div class="launch-panel">
                <button class="button icon start-icon" id="start-car-${car.id}">A</button>
                <button class="button icon stop-icon" id="stop-car-${car.id}">B</button>
              </div>
